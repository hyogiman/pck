/* 생각의 텃밭 — service worker
   ...
   v7: 기존 저장/캡처 패치 + Blooming v2 비동기 런타임.
   패치 JS는 버전 URL로 불러오고 새 SW 활성화 시 열린 앱을 한 번 새로고침합니다. */
const CACHE = "garden-v7";
const PATCH_VERSION = "20260820-1600-ai-v2";
const PATCH_TAGS = [
...
async function injectPatches(response){
  ...
  html=html.replace(/\s*<script src="\.\/(?:storage-fix|capture-marking|blooming-v2-runtime)\.js(?:\?v=[^"]*)?"><\/script>/gi,"");
...
self.addEventListener("fetch", (e) => {
  ...
  if(url.pathname.endsWith("/storage-fix.js")||url.pathname.endsWith("/capture-marking.js")||url.pathname.endsWith("/blooming-v2-runtime.js")){
    e.respondWith((async()=>{
      try{
