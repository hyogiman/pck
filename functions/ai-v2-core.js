...
function scorePass(scores) {
  ...
  if (s.grounded < 4 || s.naturalKorean < 4 || s.insightPotential < 4 || s.addsValue < 4) return false;
  if (Object.values(s).some((x) => x < 0)) return false;
  const total = Object.values(s).reduce((a, b) => a + b, 0);
  // 9 dimensions * 5 = 45. 38 keeps roughly the previous 85% bar while also
  // requiring addsValue >= 4 so a polished but unnecessary question cannot pass.
  return total >= 38;
}
